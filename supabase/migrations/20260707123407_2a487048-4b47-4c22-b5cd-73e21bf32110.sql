...
BEGIN
  ...
  INSERT INTO public.auction_events (tournament_id, event_type, player_id, team_id, price, player_name_snapshot, team_name_snapshot)
  VALUES (v_player.tournament_id, 'sale', p_player_id, p_team_id, p_price, v_player.name, v_team.name)
...
BEGIN
  ...
  INSERT INTO public.auction_events (tournament_id, event_type, player_id, price, player_name_snapshot)
  VALUES (v_player.tournament_id, 'unsold', p_player_id, NULL, v_player.name)
  RETURNING id INTO v_event_id;
  RETURN v_event_id;
END; $$;

-- ============ RESTORE UNSOLD -> AVAILABLE ============
...
BEGIN
  ...
  IF v_event.event_type = 'sale' THEN
    ...
    INSERT INTO public.auction_events (tournament_id, event_type, player_id, team_id, price, player_name_snapshot, team_name_snapshot)
    VALUES (v_event.tournament_id, 'undo_sale', v_event.player_id, v_event.team_id, v_event.price, v_event.player_name_snapshot, v_event.team_name_snapshot)
    RETURNING id INTO v_new_event_id;
  ELSE
    UPDATE public.players SET status = 'available' WHERE id = v_event.player_id;
    UPDATE public.auction_events SET is_undone = true WHERE id = v_event.id;
    INSERT INTO public.auction_events (tournament_id, event_type, player_id, player_name_snapshot)
    VALUES (v_event.tournament_id, 'undo_unsold', v_event.player_id, v_event.player_name_snapshot)
    RETURNING id INTO v_new_event_id;
  END IF;
