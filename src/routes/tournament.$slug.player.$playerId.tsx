...
function PlayerPage() {
  ...
  return (
    <div className="relative min-h-screen bg-background">
      {pageBg && (
        <div
          ...
          style={{
            ...
            backgroundSize: pageBgLayout.fit,
            backgroundPosition: pageBgLayout.position,
            opacity: 1 - pageBgLayout.overlay / 100,
          }}
        />
      )}
      ...
      {/* Compact hero — subtle, the player is the focus */}
      <div className="relative h-[120px] sm:h-[150px]">
        ...
        <div className="absolute inset-x-0 bottom-0 h-20 bg-[linear-gradient(to_top,var(--background),transparent)]" />
      </div>

      <main className="relative mx-auto -mt-16 max-w-5xl px-4 pb-14 sm:px-6">
        ...
        >
          <div className="shine relative aspect-[4/5] w-[220px] overflow-hidden rounded-2xl border-2 border-active/60 bg-muted shadow-[0_18px_50px_-22px_color-mix(in_oklab,var(--active)_85%,transparent)] sm:w-[280px]">
            {photo ? (
              <img
                src={photo}
                alt={player.name}
                  ...
                  >
                    {teamLogo ? (
                      <img
                        src={teamLogo}
                        alt=""
                        className="h-14 w-14 shrink-0 rounded-lg object-cover ring-2 ring-active/50"
                      />
                    ) : (
        ...
        {/* 3 × 3 statistics grid — every card identical */}
        <Reveal className="mt-4 grid auto-rows-fr gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          />
          <Info
            label={notAvail(player.city || ch?.city) ? "City" : "Status"}
            value={
              notAvail(player.city || ch?.city) ?? String(player.status).replace(/^\w/, (m) => m.toUpperCase())
            }
            icon={
              notAvail(player.city || ch?.city) ? <MapPin className="h-4 w-4" /> : <Activity className="h-4 w-4" />
            }
          />
        </Reveal>

        {cricheroesLink && (
          <Card className="glass-card mt-4 border-active/30">
